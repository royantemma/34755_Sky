...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  if (HEIGHT > 32) {
    ...
    drawBitmap((WIDTH - dtu_width) / 2, (HEIGHT - dtu_height) / 2,
               dtu_data, dtu_width, dtu_height, 1);
  }
#endif
...
/*!
    ...
    Display transfer is divided into 16 sections, 4 lines and sections with 32 columns
    \param page is 0..3 of the 4 lines of text
    \param column is 0..3 index to 1/4 of columns available
    All 16 sections are needed to update full display
    @return None (void).
...
void Adafruit_SSD1306::display(uint8_t page, uint8_t column) {
  ...
  dlist1[0] = SSD1306_PAGEADDR,       // (0x22, 0, 0xff)
      page,                      // Page start address
      0xFF,                   // Page end (not really, but works here)
      // SSD1306_COLUMNADDR, uint8_t(column*32)}; // Column start address (0x21, 0, width)
  ...
  // uint8_t *ptr = buffer + countFull/4*page + column*32; // send 32 bytes in one burst
  // uint16_t count = countFull / 16; // divide transfer into 4 pages with 1/4 of width
  // NB! trying with higher clock first, but 1MHz seems to be maximum
  ...
  if (false)
  {
    const int MSL = 100;
    char s[MSL];
    snprintf(s, MSL,"# display (page,col)=(%d,%d), full=%d, count=%d, offset=%d (wiremax=%d)\n",
            page,column, countFull, count, countFull/4*page + column*32/2, WIRE_MAX);
    usb.send(s);
  }
  if (wire) { // I2C
    wire->beginTransmission(i2caddr);
